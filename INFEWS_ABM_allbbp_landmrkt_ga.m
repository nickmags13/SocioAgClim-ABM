function [pop_plntdcorr_mdl,pop_croprmse_mdl,pop_FOM_mdl]=INFEWS_ABM_allbbp_landmrkt_ga(g,...
...
%     prodmapflag,bbpflag,bbpfac,uncrtythresh,simthresh,ntwkdecay]=experimental_parms(ERUNS,erun);
[neisize,farmtypeflag,pricevarflag,prodvarflag,scenarioflag,...
    prodmapflag,bbpflag,bbpfac,uncrtythresh,simthresh,ntwkdecay,...
    cropwght,plntdwght,irrwght]=experimental_parms_ga(parmfname,pitr);
...
for itcount=1:5
    for ia=1:length(ifarmer)
        ...
        [tback,~]=bbp_timehorzn(ia,ifarmer,FarmerAtt);
        ...
        if bbpflag == 1
            for jj=1:BBPobj*BBPsoc %use the same structure as batchparms
                ...
                elseif FarmerAtt.DcsnSt(ifarmer(ia))==3
                    ...
                    [N,edges]=histcounts(Tfarmprod.CropType(nfties),0.5:Ncrops+0.5);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% planted acres
plntdratio=sum(PRODACRES(Tobscrop.croptype == 1,TSTART+3:TMAX),1)./...
    sum(Tfarmprod.TotAcres(Tobscrop.croptype == 1),1);
% plntdrmse=sum(sqrt((plntdratio-plntdprop).^2);
plntdxcorr=xcorr(plntdprop,plntdratio,0);

% crop type
mdl_lutype=zeros(height(Tobscrop),1);
mdl_lutype(ismember(CROPHIST(:,TMAX),1:4),1)=1;
mdl_lutype(ismember(CROPHIST(:,TMAX),5:7))=2;
mdl_lutype(CROPHIST(:,TMAX)==8)=3;
croprmse=sum(sqrt((mdl_lutype-Tobscrop.croptype).^2));

yr=2000:2015;
obs_irrac=repmat(Tfarmprod.TotAcres,1,length(yr)).*(table2array(Tirr(:,2:width(Tirr)-1)) > 0);
...
for gg=1:length(obs_irrac)
    if finalmdl_irrac(gg) > 0 && obs_irrac(gg) > 0
        errclass(gg)=1;
    elseif finalmdl_irrac(gg) == 0 && obs_irrac(gg) == 0
        errclass(gg)=2;
    elseif finalmdl_irrac(gg) > 0 && obs_irrac(gg) == 0
        errclass(gg)=3;
    elseif finalmdl_irrac(gg) == 0 && obs_irrac(gg) > 0
...
FOM_precision=length(find(errclass == 1))/max(length(find(errclass == 1))+...
    length(find(errclass == 3)),1);
FOM_recall=length(find(errclass == 1))/max(length(find(errclass == 1))+...
    length(find(errclass == 4)),1);       %change prediction accuracy
FOM_neg=length(find(errclass == 3))/max(length(find(errclass == 3))+...
    length(find(errclass == 2)),1);      %false alarm rate
FOM_global=(length(find(errclass == 1))+length(find(errclass == 2)))/height(Tfarmprod);


% savefname=sprintf('infewsabm_results_allvar_bbp%d_02182024_%d_%d',...
%     erun,mrun,hubid);
...
% hubstr=sprintf('allbbp_hub%dlandmrkt_allvar_osse_ga_04152024\',hubid);
% savefname=sprintf('%s%sga_results_%d_%d',...
%     'C:\Users\nrmagliocca\Box\Socio-Agroclimatology\ABM_Drive\Results\',hubstr,g,pitr);
...
savefname=sprintf('%sga_results_%d_%d',hubstr,g,pitr);

% savefname=sprintf('%sga_results_%d_%d',...
%     'C:\Users\nrmagliocca\Box\INFEWS_Project\ABM_drive\Results\allbbp_hubs_landmrkt_allvar_osse_ga_02262024\',g,pitr);
parsave_infewsabm_landmrkt_ga(savefname,plntdxcorr,croprmse,FOM_global,...
    FOM_precision,FOM_recall,FOM_neg);


pop_plntdcorr_mdl=zeros(POP,1);
pop_croprmse_mdl=zeros(POP,1);
pop_FOM_mdl=zeros(POP,4);
for ip=1:POP
    %load resutls files
    % fname=sprintf('%sga_results_%d_%d',...
    %     'C:\Users\nrmagliocca\Box\INFEWS_Project\ABM_drive\Results\allbbp_hubs_landmrkt_allvar_osse_ga_02262024\',g,ip);
    % hubstr=sprintf('allbbp_hub%dlandmrkt_allvar_osse_ga_04152024\',hubid);
    % fname=sprintf('%s%sga_results_%d_%d',...
    %     'C:\Users\nrmagliocca\Box\Socio-Agroclimatology\ABM_Drive\Results\',hubstr,g,ip);
    % hubstr=sprintf('C:/Users/nrmagliocca/Box/Socio-Agroclimatology/ABM_Drive/Results/allbbp_hub%d_landmrkt_allvar_osse_ga_04152024/',hubid);
    hubstr=sprintf('C:/Users/nrmagliocca/Box/Socio-Agroclimatology/ABM_Drive/Results/allbbp_hub%d_landmrkt_allvar_osse_ga_06042024/',hubid);
    fname=sprintf('%sga_results_%d_%d',hubstr,g,ip);
    load(fname)

    pop_plntdcorr_mdl(ip)=plntdxcorr_save;
    pop_croprmse_mdl(ip)=croprmse_save;
    pop_FOM_mdl(ip,:)=FOM_stats;
end